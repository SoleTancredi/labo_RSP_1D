...
static int addNode(LinkedList *this, int nodeIndex, void *pElement)
{
    ...
    if (this != NULL && newNode!=NULL && nodeIndex >-1 && nodeIndex <= ll_len(this))
    {
		...
		len = ll_len(this);
			if (nodeIndex == 0) // si la lista no tiene nada
			{
				(*newNode).pNextNode = (*this).pFirstNode;  //primero se lo cargo a new lo de first
			...
			else
			{
				if( nodeIndex <= len )// 5
				{
					pPreviousNode = getNode(this, nodeIndex-1); // 4
					newNode->pNextNode = pPreviousNode->pNextNode;
					pPreviousNode->pNextNode = newNode;
				}
			}
			this->size++;
		returnAux = 0;
...
{
    LinkedList* retornoGalgo = NULL;
    Perrito* pElement;

    retornoGalgo = ll_newLinkedList(); // creo lista nueva
    if(this != NULL && pFunc != NULL && retornoGalgo !=NULL)
    {
		for(int i = 0; i < ll_len(this); i++)
		{
			pElement = ll_get(this, i);
			if(pFunc(pElement) == 0)
			{
				ll_add(retornoGalgo,pElement);// lista  limpia
				perrito_showUnitPunto3(pElement);
			}
